...
export default function NextRoadTrip({ img, title, text, button, href }) {
  return (
    <div className="group h-125 md:h-150 relative bg-slate-800 overflow-hidden  transition-all duration-500">
      <Image
        src={img}
        alt={title}
        fill
        className="object-cover transition-transform duration-[1.5s] group-hover:scale-103 opacity-70 md:opacity-75"
        priority
      />
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-16 text-white">
        <div className="max-w-3xl space-y-4 md:space-y-6">
          ...
          <h2 className="text-5xl md:text-5xl font-bold tracking-tight leading-[0.9] uppercase ">
            {title}
          </h2>
          ...
          <div className="pt-4">
            <Link href={href}>
              <button className="group/btn relative bg-white hover:bg-amber-400 text-slate-900 font-black uppercase text-xs md:text-sm tracking-widest py-4 px-8 md:px-10  rounded-xl transition-all duration-300 flex items-center gap-3 hover:cursor-pointer">
                {button}
